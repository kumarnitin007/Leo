    ...
    <div>
      ...
      {isMobile ? (
        <div style={{display:"flex",flexDirection:"column",gap:12}}>
          ...
          <div style={{background:"var(--ck-ink)",borderRadius:14,padding:"16px",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
            <div>
          ...
          <div style={{display:"flex",gap:6,overflowX:"auto",paddingBottom:4,WebkitOverflowScrolling:"touch"}}>
            <button onClick={() => setFilterBank("All")} style={{background:filterBank === "All" ? "var(--ck-purple)" : THEME.cardBgAlt,color:filterBank === "All" ? "#FFF" : THEME.textMuted,border:"none",borderRadius:20,padding:"8px 14px",fontSize:12,fontWeight:600,flexShrink:0}}>All ({deposits.length})</button>
            {depBankNames.map(b => (
          ...
          {filtered.length === 0 ? (
            <div style={{padding:40,textAlign:"center",color:"var(--ck-ink3)"}}>No deposits found</div>
          ) : (
                  ...
                  >
                    <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:10}}>
                      <div>
                        ...
                        <div style={{fontSize:11,color:"var(--ck-ink3)",marginTop:2}}>{d.type || "FD"} {d.depositId && `• ${d.depositId}`}</div>
                      </div>
                      {days !== null && !d.done && (
                        <div style={{
                          background: urgency === "urgent" ? "var(--ck-red-light)" : urgency === "soon" ? "var(--ck-gold-light)" : urgency === "expired" ? "var(--ck-cream)" : THEME.cardBgAlt,
                          color: urgency === "urgent" ? "var(--ck-red)" : urgency === "soon" ? "var(--ck-gold)" : urgency === "expired" ? THEME.textLight : THEME.textMuted,
                          padding:"4px 10px",
                          borderRadius:20,
                          fontSize:11,
                          fontWeight:600
                      ...
                      {d.done && <span style={{fontSize:11,color:"var(--ck-green)",fontWeight:600}}>✓ Done</span>}
                    </div>
                    
                    <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-end"}}>
                      <div>
                        <div style={{fontSize:11,color:"var(--ck-ink3)"}}>Invested</div>
                      ...
                      <div style={{textAlign:"right"}}>
                        <div style={{fontSize:11,color:"var(--ck-ink3)"}}>Maturity</div>
                    ...
                    {d.nextAction && !d.done && (
                      <div style={{fontSize:11,color:"var(--ck-purple)",marginTop:10,fontWeight:600}}>📌 Next: {d.nextAction}</div>
                    )}
                    ...
                    <div style={{display:"flex",gap:8,marginTop:12,paddingTop:12,borderTop:`1px solid ${THEME.border}`}}>
                      <button 
                        ...
                        style={{flex:1,background:d.done ? "var(--ck-green-light)" : THEME.cardBgAlt,color:d.done ? "var(--ck-green)" : THEME.textLight,border:"none",borderRadius:8,padding:"10px",fontSize:12,fontWeight:600}}
                      >
                        {d.done ? "↩ Undo" : "✓ Mark Done"}
                      </button>
                      <button 
                        onClick={() => openEdit("deposit", origIdx)} 
                        style={{background:"var(--ck-purple-light)",color:"var(--ck-purple)",border:"none",borderRadius:8,padding:"10px 14px",fontSize:12}}
                      >✏️</button>
                      <button 
                        onClick={() => deleteRow("deposit", origIdx)} 
                        style={{background:"var(--ck-red-light)",color:"var(--ck-red)",border:"none",borderRadius:8,padding:"10px 14px",fontSize:12}}
                      >🗑</button>
                    </div>
                  </div>
                );
        ...
        <div style={{display:"flex",gap:8,flexWrap:"wrap",alignItems:"center"}}>
          ...
          <div style={{display:"flex",gap:1,background:THEME.cardBg,borderRadius:6,padding:2,border:`1px solid ${THEME.border}`}}>
            <button
              ...
              style={{background:depositsViewMode === 'cards' ? 'var(--ck-purple)' : 'transparent',color:depositsViewMode === 'cards' ? '#FFF' : 'var(--ck-ink3)',border:'none',padding:'4px 10px',borderRadius:4,fontSize:10,fontWeight:600,cursor:'pointer'}}
            ...
            <button
              ...
              style={{background:depositsViewMode === 'grouped' ? 'var(--ck-purple)' : 'transparent',color:depositsViewMode === 'grouped' ? '#FFF' : 'var(--ck-ink3)',border:'none',padding:'4px 10px',borderRadius:4,fontSize:10,fontWeight:600,cursor:'pointer'}}
            ...
            <button
              ...
              style={{background:depositsViewMode === 'flat' ? 'var(--ck-purple)' : 'transparent',color:depositsViewMode === 'flat' ? '#FFF' : 'var(--ck-ink3)',border:'none',padding:'4px 10px',borderRadius:4,fontSize:10,fontWeight:600,cursor:'pointer'}}
          ...
          <div style={{display:"flex",gap:4,flexWrap:"wrap",flex:isMobile?undefined:1}}>
            <button onClick={() => setFilterBank("All")} style={{background:filterBank === "All" ? "var(--ck-purple)" : THEME.cardBgAlt,color:filterBank === "All" ? "#FFF" : THEME.textMuted,border:"none",borderRadius:16,padding:"5px 10px",fontSize:10,fontWeight:600,cursor:"pointer"}}>All</button>
            {banks.map(b => (
                      ...
                      >
                        <td style={{padding:"8px 10px"}}>
                          <div style={{display:"flex",alignItems:"center",gap:8}}>
                            ...
                            <span style={{color:"var(--ck-ink3)",fontSize:10}}>({bankDeps.length} FD{bankDeps.length > 1 ? "s" : ""})</span>
                          </div>
                          {isSingleRow && singleDep?.depositId && <div style={{fontSize:9,color:"var(--ck-ink3)",fontFamily:"monospace",marginLeft:30}}>{singleDep.depositId}</div>}
                        </td>
                        ...
                        <td style={{padding:"8px 10px",whiteSpace:"nowrap"}}>
                          {isSingleRow && (
                            <>
                              <button onClick={(e) => { e.stopPropagation(); toggleDone("deposit", indices[0]); }} style={{background:singleDep?.done ? "var(--ck-green)" : THEME.cardBgAlt,color:singleDep?.done ? "#fff" : THEME.textMuted,border:"none",borderRadius:4,padding:"2px 6px",fontSize:9,cursor:"pointer",marginRight:3,fontWeight:600}}>{singleDep?.done ? "↩" : "✓"}</button>
                              <button onClick={(e) => { e.stopPropagation(); openEdit("deposit", indices[0]); }} style={{background:THEME.cardBgAlt,color:"var(--ck-purple)",border:"none",borderRadius:4,padding:"2px 6px",fontSize:9,cursor:"pointer",marginRight:3}}>✏️</button>
                              <button onClick={(e) => { e.stopPropagation(); deleteRow("deposit", indices[0]); }} style={{background:THEME.cardBgAlt,color:"var(--ck-red)",border:"none",borderRadius:4,padding:"2px 6px",fontSize:9,cursor:"pointer"}}>🗑</button>
                            </>
                          )}
                        </td>
                      </tr>
                        ...
                        return (
                          <tr key={`${bankName}-${j}`} style={{borderBottom:`1px solid ${THEME.border}`,background:d.done ? "rgba(46,160,67,0.05)" : days != null && days < 0 ? "rgba(110,118,129,0.1)" : days != null && days <= 90 ? "rgba(248,81,73,0.05)" : "transparent",opacity:d.done ? 0.6 : 1}}>
                            <td style={{padding:"8px 10px",paddingLeft:32}}>
                              <div style={{fontSize:9,color:"var(--ck-ink3)",fontFamily:"monospace"}}>{d.depositId || "—"}</div>
                            </td>
                            ...
                            <td style={{padding:"8px 10px",color:"var(--ck-ink3)",fontSize:9}}>{d.duration || "—"}</td>
                            <td style={{padding:"8px 10px",color:d.nextAction && !d.done ? "var(--ck-gold)" : THEME.textLight,fontSize:9,maxWidth:72,overflow:"hidden",textOverflow:"ellipsis",fontWeight:d.nextAction && !d.done ? 600 : 400}} title={d.nextAction || ""}>{d.nextAction ? (d.done ? "✓ " : "⚡ ") + d.nextAction : "—"}</td>
                            ...
                            <td style={{padding:"8px 10px",whiteSpace:"nowrap"}}>
                              <button onClick={() => toggleDone("deposit", origIdx)} style={{background:d.done ? "var(--ck-green)" : THEME.cardBgAlt,color:d.done ? "#fff" : THEME.textMuted,border:"none",borderRadius:4,padding:"2px 6px",fontSize:9,cursor:"pointer",marginRight:3,fontWeight:600}}>{d.done ? "↩" : "✓"}</button>
                              <button onClick={() => openEdit("deposit", origIdx)} style={{background:THEME.cardBgAlt,color:"var(--ck-purple)",border:"none",borderRadius:4,padding:"2px 6px",fontSize:9,cursor:"pointer",marginRight:3}}>✏️</button>
                              <button onClick={() => deleteRow("deposit", origIdx)} style={{background:THEME.cardBgAlt,color:"var(--ck-red)",border:"none",borderRadius:4,padding:"2px 6px",fontSize:9,cursor:"pointer"}}>🗑</button>
                            </td>
                          </tr>
                        );
                      })}
                ...
                filtered.map((d, idx) => {
                  ...
                  return (
                    <tr key={idx} style={{borderBottom:`1px solid ${THEME.border}`,background:d.done ? "rgba(46,160,67,0.05)" : days != null && days < 0 ? "rgba(110,118,129,0.1)" : days != null && days <= 90 ? "rgba(248,81,73,0.05)" : "transparent",opacity:d.done ? 0.6 : 1}}>
                      ...
                      <td style={{padding:"8px 10px",fontSize:9,color:"var(--ck-ink3)",fontFamily:"monospace"}}>{d.depositId || "—"}</td>
                      <td style={{padding:"8px 10px",color:THEME.textLight,fontSize:10}}>{d.type || "FD"}</td>
                            ...
                            <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
                              <div style={{flex:1}}>
                                <div style={{display:"flex",alignItems:"center",gap:8,flexWrap:"wrap"}}>
                                  {d.depositId && <span style={{fontSize:9,color:"var(--ck-ink3)",fontFamily:"monospace"}}>{d.depositId}</span>}
                                  <span style={{background:"var(--ck-purple-light)",color:"var(--ck-purple)",padding:"2px 6px",borderRadius:4,fontSize:9,fontWeight:600}}>{d.type || "FD"}</span>
                                  {d.accountOwner && <span style={{fontSize:9,color:"var(--ck-ink3)"}}>Owner: {d.accountOwner}</span>}
                                  {d.nominee && <span style={{fontSize:9,color:"var(--ck-ink3)"}}>👤 {d.nominee}</span>}
                                  {d.done && <span style={{fontSize:9,color:"var(--ck-green)"}}>✓ Done</span>}
                                </div>
                                <div style={{display:"flex",alignItems:"baseline",gap:12,marginTop:6,flexWrap:"wrap"}}>
                                  <div>
                                    <span style={{fontSize:9,color:"var(--ck-ink3)"}}>Invested</span>
                                    <div style={{fontSize:13,fontWeight:700,fontFamily:"monospace",color:THEME.text}}>{fmt(d.deposit)}</div>
                                  </div>
                                  <div>
                                    <span style={{fontSize:9,color:"var(--ck-ink3)"}}>ROI</span>
                                    <div style={{fontSize:11,fontWeight:600,fontFamily:"monospace",color:THEME.accent}}>{d.roi ? (Number(d.roi) * 100).toFixed(2) + "%" : "—"}</div>
                                  </div>
                                  <div>
                                    <span style={{fontSize:9,color:"var(--ck-ink3)"}}>Maturity</span>
                                ...
                                {d.balanceHistory?.length > 0 && (() => {
                                  ...
                                  return (
                                    <div style={{fontSize:10,color:"var(--ck-ink3)",marginTop:4}} title={d.balanceHistory.map((h: { date: string; amount: number; previousAmount?: number; source?: string }) => `${new Date(h.date).toLocaleString()}: ${h.previousAmount != null ? fmt(h.previousAmount, (d.currency || 'INR') as Currency) + ' → ' : ''}${fmt(h.amount, (d.currency || 'INR') as Currency)} ${h.source || ''}`).join('\n')}>
                                      📅 Updated {new Date(latest.date).toLocaleDateString(undefined, { dateStyle: 'short' })} · {prev}{fmt(latest.amount, (d.currency || 'INR') as Currency)} {latest.source && <span style={{color:"var(--ck-ink3)"}}>({latest.source})</span>}
                                    </div>
                                  );
                                })()}
                                <div style={{display:"flex",gap:12,marginTop:6,fontSize:10,color:THEME.textLight}}>
                                  ...
                                  {d.duration && <span style={{color:"var(--ck-ink3)"}}>{d.duration}</span>}
                                  <UrgencyBadge days={days} />
                                </div>
                                {d.nextAction && !d.done && <div style={{fontSize:10,color:"var(--ck-purple)",marginTop:4,fontWeight:600}}>📌 {d.nextAction}</div>}
                                {d.maturityAction && <div style={{fontSize:10,color:"var(--ck-gold)",marginTop:4}}>⚡ {d.maturityAction}</div>}
                              </div>
                              <div style={{display:"flex",gap:4,flexShrink:0}}>
                                <button onClick={(e) => { e.stopPropagation(); toggleDone("deposit", origIdx); }} style={{background:d.done ? "var(--ck-green-light)" : THEME.cardBgAlt,color:d.done ? "var(--ck-green)" : "var(--ck-ink3)",border:`1px solid ${d.done ? "var(--ck-green)" : THEME.border}`,borderRadius:5,padding:"2px 6px",fontSize:10,cursor:"pointer",fontWeight:700}}>{d.done ? "↩" : "✓"}</button>
                                <button onClick={(e) => { e.stopPropagation(); openEdit("deposit", origIdx); }} style={{background:"var(--ck-purple-light)",color:"var(--ck-purple)",border:"1px solid rgba(107,93,232,0.4)",borderRadius:5,padding:"2px 5px",fontSize:10,cursor:"pointer"}}>✏️</button>
                                <button onClick={(e) => { e.stopPropagation(); deleteRow("deposit", origIdx); }} style={{background:"var(--ck-red-light)",color:"var(--ck-red)",border:"1px solid rgba(201,74,46,0.4)",borderRadius:5,padding:"2px 5px",fontSize:10,cursor:"pointer"}}>🗑</button>
                              </div>
                            </div>
                          </div>
                        );
