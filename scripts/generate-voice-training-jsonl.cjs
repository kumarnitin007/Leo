...
function parseValue(raw) {
  ...
  // Keep HIGH/MEDIUM/LOW as raw tokens; confidence is handled by the CONFIDENCE key.
  return v;
}
...
function parseBlock(block) {
  ...
  const tokens = normalized
    ...
    .map((t) => t.trim())
    .filter(Boolean)
    // ignore code fence terminators and inline markdown comments
    .filter((t) => !t.startsWith('```') && !t.startsWith('--'));
  for (const tok of tokens) {
    const eqIdx = tok.indexOf('=');
    if (eqIdx === -1) continue;
    const key = tok.slice(0, eqIdx).trim();
    let rawVal = tok.slice(eqIdx + 1).trim();
    // drop any trailing inline comments or code fence parts
    rawVal = rawVal.replace(/```/g, '').trim();
    // strip trailing inline comments indicated by --
    const commentIdx = rawVal.indexOf('--');
    if (commentIdx !== -1) rawVal = rawVal.slice(0, commentIdx).trim();
...
function generate() {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (line.startsWith('// Example')) {
      // find next quoted block
      let j = i + 1;
      while (j < lines.length && lines[j].trim() === '') j++;
      if (j >= lines.length) break;
      // capture block starting at the quoted line until an empty line
      if (!lines[j].includes('::')) {
        // find the line that contains the quoted utterance with ::
        let k = j;
      ...
      if (!parsed.rawTranscript || parsed.rawTranscript.includes('<')) continue;
      out.push(parsed);
      i = k;
    }
  }
